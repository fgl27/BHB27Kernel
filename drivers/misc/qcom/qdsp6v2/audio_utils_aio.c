...
static void audio_aio_unmap_ion_region(struct q6audio_aio *audio)
{
	...
	list_for_each_safe(ptr, next, &audio->ion_region_queue) {
		region = list_entry(ptr, struct audio_aio_ion_region, list);
		if (region != NULL) {
			pr_debug("%s[%pK]: phy_address = 0x%pKa\n",
				...
				void *vaddr, unsigned long len)
{
	...
	list_for_each_entry(region_elt, &audio->ion_region_queue, list) {
		if (CONTAINS(region_elt, &t) || CONTAINS(&t, region_elt) ||
			...
			pr_err("%s[%pK]:region (vaddr %pK len %ld) clashes with registered region (vaddr %pK paddr %pKa len %ld)\n",
				...
				struct msm_audio_ion_info *info)
{
	...
	region->len = len;
	region->ref_cnt = 0;
	pr_debug("%s[%pK]:add region paddr %pKa vaddr %pK, len %lu kvaddr %pK\n",
		__func__, audio,
				...
				struct audio_aio_buffer_node *buf_node)
{
	...
	pr_debug("%s[%pK]: Send write buff %pK phy %pKa len %d meta_enable = %d\n",
				...
				struct audio_aio_buffer_node *buf_node)
{
	...
	pr_debug("%s[%pK]: Send read buff %pK phy %pKa len %d\n",
		__func__, audio, buf_node,
