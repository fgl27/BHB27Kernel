			...
			int len, int type, u8 *read)
{
	...
	err = codec->bulk_write_raw(codec, TFA9890_CF_CONTROLS, buffer,
						ARRAY_SIZE(buffer));
	if (err < 0) {
		pr_err("%s: Failed to Write DSP controls req: %d\n", __func__, err);
		return err;
	}
	/* check transfer type */
	...
	if ((cf_status & TFA9890_STATUS_CF) == 0) {
		pr_err("%s: Failed to ack DSP CTL req: %d\n", __func__, err);
		return -EIO;
	}
	/* read the RPC Status */
	...
	if (err < 0) {
		pr_err("%s: Failed to Write NXP DSP CTL reg for rpc check: %d\n",
				__func__, err);
		return err;
	}
	...
	if (rpc_status != TFA9890_STATUS_OK) {
		pr_err("%s: RPC status check failed: %d\n", __func__,
				err);
		return -EIO;
	}
...
static int tfa9890_set_config_right(struct tfa9890_priv *tfa9890)
{
	...
	if (!right_fw_config[tfa9890->cfg_mode] ||
				...
				TFA9890_CFG_FW_SIZE)) {
			pr_err("%s: Data size check failed cfg file\n", __func__);
			return -EIO;
	}
	...
	ret = tfa9890_dsp_transfer(tfa9890->codec,
				...
				TFA9890_DSP_WRITE, 0);
	if (ret < 0) {
		pr_err("%s: Failed to load cfg for mode: %d\n", __func__,
			tfa9890->cfg_mode);
		return ret;
	}
...
static int tfa9890_set_mode_right(struct tfa9890_priv *tfa9890)
{
	...
	if (!right_fw_pst_table[tfa9890->mode] ||
		...
		pr_err("%s: Data size check failed preset file\n", __func__);
		return -EIO;
	}

	pr_debug("%s: switching to mode: %s vol idx: %d\n", __func__,
	...
	if (tfa9890->update_eq) {
		if (!right_fw_eq[tfa9890->mode] ||
			...
			pr_err("%s: Data size check failed eq file\n", __func__);
			return -EIO;
		}
		...
		ret = tfa9890_dsp_transfer(tfa9890->codec,
			...
			TFA9890_DSP_WRITE, 0);
		if (ret < 0) {
			pr_err("%s: Failed to load eq for mode: %s\n", __func__,
				tfa9890_mode[tfa9890->mode]);
			return ret;
		}
		pr_debug("%s: Switching eq file to mode: %s\n", __func__,
			tfa9890_mode[tfa9890->mode]);
	}
		...
		struct snd_ctl_elem_value *ucontrol)
{
	...
	if ((vol_value > tfa9890->max_vol_steps || vol_value < 0) ||
			...
			mode_value < 0)) {
		pr_err("%s: Invalid mode or vol index set\n", __func__);
...
static int tfa9890_get_ic_ver(struct snd_soc_codec *codec)
{
	...
	if ((ver1 == TFA9890_N1B12_VER1_VAL) &&
		...
		pr_debug("%s: n1b12 version detected\n", __func__);
		ret = TFA9890_N1B12;
	} else if (!ver1 && !ver2 && !ver3) {
		pr_debug("%s: n1b4 version detected\n", __func__);
		ret = TFA9890_N1B4;
	} else
		/* if it not above types then it must be N1C2 ver type */
		pr_debug("%s: n1c2 version detected\n", __func__);
...
static int tfa9890_wait_pll_sync(struct tfa9890_priv *tfa9890)
{
	...
	if (tries == PLL_SYNC_RETRIES) {
		pr_err("%s: DSP pll sync failed: %s\n", __func__, codec->name);
		ret = -EIO;
	}
				...
				int fw_len)
{
	...
	while (index < length) {
		...
		if ((size) > TFA9890_MAX_I2C_SIZE) {
			/* too big */
			pr_err("%s: Invalid dsp patch\n", __func__);
			goto out;
		}
		err = codec->bulk_write_raw(codec, *(fw_data + index),
				(fw_data + index), size);
		if (err < 0) {
			pr_err("%s: Writing dsp patch failed\n", __func__);
			goto out;
		}
		index += size;
	}
...
static int tfa9890_load_config(struct tfa9890_priv *tfa9890)
{
	...
	if (!fw_name) {
		pr_err("%s: Failed to allocate fw_name\n", __func__);
		goto out;
	}
	...
	if (tfa9890->ic_version == TFA9890_N1C2) {
		...
		if (ret) {
			pr_err("%s: Failed to locate tfa9890_n1c2.patch\n", __func__);
			goto out;
		}
		...
		ret = request_firmware(&fw_patch, fw_name, codec->dev);
		if (ret) {
			pr_err("%s: Failed to locate tfa9890_n1b12.patch\n", __func__);
			goto out;
		}
	...
	if (fw_patch) {
		ret = tfa9887_load_dsp_patch(codec, fw_patch->data,
				fw_patch->size);
		if (ret) {
			pr_err("%s: Failed to load dsp patch!!\n", __func__);
			goto out;
		}
	}
	...
	if (ret) {
		pr_err("%s: Failed to locate speaker model!!\n", __func__);
		goto out;
	}
	if (fw_speaker->size != TFA9890_SPK_FW_SIZE) {
		pr_err("%s: Data size check failed for spkr model\n", __func__);
		goto out;
	}
...
static void tfa9890_load_preset(struct work_struct *work)
{
	...
	mutex_lock(&lr_lock);
	pr_info("%s: %s loading presets,eq and config files\n", __func__, codec->name);
	...
	if (!preset_name || !cfg_name || !eq_name) {
		...
		pr_err("%s: Load preset allocation failure\n", __func__);
		kfree(preset_name);
		kfree(cfg_name);
		kfree(eq_name);
		mutex_unlock(&lr_lock);
	...
	for (i = 0; i < ARRAY_SIZE(tfa9890_mode); i++) {
		...
		if (!strncmp("left", tfa9890->tfa_dev, 4)) {
			...
			if (ret || left_fw_config[i]->size
				!= TFA9890_CFG_FW_SIZE) {
				pr_err("%s: Failed to load dsp config!!\n", __func__);
				tfa9890->dsp_init = TFA9890_DSP_INIT_FAIL;
			}
		} else {
...
static void tfa9890_dsp_init(struct work_struct *work)
{
	...
	pr_info("%s: Initializing DSP, status:0x%x codec name %s\n",
				__func__, val, codec->name);

	/* cold boot device before loading firmware and parameters */
	ret = tfa9890_coldboot(codec);
	if (ret < 0) {
		pr_err("%s: Cold boot failed!!\n", __func__);
		goto out;
	}

	ret = tfa9890_load_config(tfa9890);
	if (ret < 0) {
		pr_err("%s: Load dsp config failed!!\n", __func__);
		goto out;
	}
	...
	if (!(val & TFA9890_STATUS_MTPEX)) {
		pr_info("%s: Calib not completed initiating..\n", __func__);
			     ...
			     struct snd_soc_dai *dai)
{
	...
	if (params_format(params) != SNDRV_PCM_FORMAT_S16_LE) {
		pr_err("%s: Invalid pcm bit length %i\n", __func__,
	...
	if (bclk_ws_ratio != 32 && bclk_ws_ratio != 48
			&& bclk_ws_ratio != 64) {
		pr_err("%s: Invalid bit clk to ws freq ratio: %d\n", __func__,
			bclk_ws_ratio);
		return -EINVAL;
	}
				      ...
				      const struct i2c_device_id *id)
{
	...
	pr_info("%s: %s probed successfully!\n", __func__, tfa9890->tfa_dev);
